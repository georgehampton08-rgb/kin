...
### 2e. Make `created_by` NOT NULL Again (Optional)
...
| `GET /devices` (valid token)              | 200 + family-scoped device list     |
...
6. `backend/app/api/v1/endpoints/devices.py` — restore Depends
7. `backend/app/api/v1/endpoints/auth.py` — restore user scope
8. `backend/app/models/location.py` — optional NOT NULL restore
